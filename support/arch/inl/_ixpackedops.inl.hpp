...
template<> struct simd_op<float, 4> : public simd_op_base<float, 4>
{
	...
	{
		return  base_t::_Binary([&]()->type{return _mm_and_ps(_Right, _mm_castsi128_ps(_mm_setr_epi32(-1, 0x7FFFFFFF, -1, 0x7FFFFFFF))); });
	}
};
template<> struct simd_op<float, 8> : public simd_op_base<float, 8>
{
	...
	HOST_STATIC_INL_CXPR_T const abs(const type& _Right)
	{
		return base_t::_Binary([&]()->type{ return _mm256_and_ps(_Right, _mm256_castsi256_ps(_mm256_setr_epi32(-1, 0x7FFFFFFF, -1, 0x7FFFFFFF, -1, 0x7FFFFFFF, -1, 0x7FFFFFFF))); });
	}
};
...
template<> struct simd_op<double, 4> : public simd_op_base<double, 4>
{
	...
	HOST_STATIC_INL_CXPR_T const abs(const type& _Right) 
	{
		return base_t::_Binary([&]()->type{ return _mm256_and_pd(_Right, _mm256_castsi256_pd(_mm256_setr_epi32(-1, 0x7FFFFFFF, -1, 0x7FFFFFFF, -1, 0x7FFFFFFF, -1, 0x7FFFFFFF))); });
	}
};
